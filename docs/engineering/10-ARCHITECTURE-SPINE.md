...
### ADR-2 · 죽어도 잃지 마라 — 담기가 분석보다 먼저다 (개정)

> 이 ADR 의 제목은 한때 "Share Extension 은 수집 전용" 이었다. 지금 Extension 은
> 등록까지 끝낸다. **막는 대상이 바뀐 것이지 원칙이 바뀐 것은 아니다** —
> 알맹이는 "무거운 것 금지" 가 아니라 **"죽어도 잃지 마라"** 였다.
> `SharedInbox.enqueue` 가 **먼저** 오면 시스템이 확장을 죽여도 캡처가 남고,
> 앱이 다음에 이어서 처리한다. 같은 이유로 `CaptureQueue` 도 줄에 세우기 전에 담는다.
...
### ADR-4 · AI 는 제안한다 — 사람이 판단하는 자리는 저장 **뒤**다 (개정)

**처음 결정** — 확인 화면을 **항상** 거친다.

**왜 바꿨나** — 그 흐름은 넷이었다. 공유 → 담기 → 앱 열기 → 확인.
그 사이 어디서든 잊으면 아무 일도 일어나지 않았고, 담아 두기만 한 스크린샷은
사진첩에 묻힌 것과 다를 게 없었다. **확인 화면이 지킨 것은 정확성이 아니라
아무 일도 안 일어나는 상태**였다.

**지금 결정** — 사람이 최종 판단을 하는 자리를 저장 **앞**에서 저장 **뒤**로 옮긴다.

```
분명할 때   확인 없이 등록한다  →  "언제 무슨 일정이 등록되었습니다" 알림
모호할 때   등록하지 않는다     →  "언제인지 확인해 주세요" 알림
```

판정은 `AutoFilePolicy` 순수 함수가 한다. 날짜가 없거나, 모호점이 있거나,
`confidence < 0.80` 이면 등록하지 않는다.

**그래서 반드시 있어야 하는 것 둘**

| **알린다** | 등록·미등록·실패 전부. 알리지 않는 자동 등록은 몰래 쓴 일정이다 |
| **고칠 수 있다** | 어디서든 · 언제든. 고칠 수 없는 자동 등록은 남이 쓴 일정이다 |

둘 다 없으면 이 ADR 은 그냥 "확인을 없앴다" 가 된다. 그건 다른 결정이고 나쁜 결정이다.

**한때 있었던 배너를 없앤 이유** — 맥에서 8초짜리 "실행 취소" 배너를 띄웠었다.
배너는 **떠 있는 동안만** 결정권을 준다. 커피를 가지러 간 사이 사라지면 남는 게 없다.
알림은 알림 센터에 남고, 등록된 것은 언제든 고칠 수 있다 — 되돌릴 시간이
8초에서 사실상 무제한이 됐다.
...
| --- | --- | --- |
| 앱 할 일 | 원장 | ✅ |
| Apple 캘린더 | 선택적 출력 | ❌ (앱에서 고치면 따라간다 · 지우면 거둔다) |
| 캘린더 탭 | 원장을 날짜로 본 시점 | ✅ **화면에서는** — 저장은 원장으로 간다 |
| 로컬 알림 | 원장에서 파생된 예약 | ❌ (자동 재계산) |

**왜** — 두 곳에서 고칠 수 있으면 "어느 쪽이 진짜인가"를 사용자도 코드도 답하지 못한다.
캘린더 저장 실패가 할 일 저장을 되돌리지 않는 것도 같은 이유다.

**개정 — 캘린더 탭에서도 고칠 수 있다.** "보기만 하는 화면" 과 "고칠 수 없는 화면" 은
다른 말이다. 날짜를 눌러 일정을 찾아낸 사용자가 거기서 고칠 수 없으면 목록에서
**같은 것을 다시 찾아야** 한다.

원장이 둘이 되는 것은 **화면이 둘일 때가 아니라 저장 경로가 둘일 때**다.
그래서 화면은 여럿이되 문은 하나다 — `TaskEditorSheet` → `TaskStore.commit`.

**대가** — 캘린더 앱에서 일정을 고쳐도 앱에 반영되지 않는다. 역동기화는 하지 않는다.
반대 방향(앱 → 캘린더)은 `TaskStore.update` 가 맞춘다.
