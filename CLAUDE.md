...
5. **Share Extension은 수집에 집중한다.** 긴 네트워크 요청·EventKit·알림을 넣지 않는다. 시스템이 중간에 죽인다
...
11. **알림 예약은 `LocalNotificationService` 한 곳에서만.** 화면이 직접 걸면 취소 지점이 갈라지고 끝낸 일이 계속 울린다
...
**알림 시각** — `ReminderSchedule` 이 계산하고 서비스는 걸기만 한다.
...
R0 실행 뼈대 + MVP 기능 + 접근성 완성 · **iOS 테스트 73건 · 백엔드 테스트 15건 · 빌드 경고 0**
