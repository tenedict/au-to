...
1. **AI 결과는 제안이다 — 사람이 최종 판단을 하는 자리는 저장 뒤다.**
   분명하면 확인 없이 등록하고 **등록했다고 알린다**. 모호하면 등록하지 않고 확인을 요청한다 (`AutoFilePolicy`).
   대신 **등록된 것은 언제 어디서든 고치고 지울 수 있어야 한다** — 고칠 수 없는 자동 등록은 그냥 남이 쓴 일정이다.
   등록·수정·삭제를 알리지 않는 경로를 만들지 않는다
...
저장된 할 일과 확인 대기 초안이 **같은 편집기**를 지난다. 나눠 만들면 한쪽에만
있는 항목이 반드시 생긴다 (실제로 한동안 저장한 할 일은 고칠 수 없었다).
...
확인 요청  CaptureNotice 가 소유 · 식별자 앞에 "capture#"
  postNeedsReview   날짜가 모호할 때. 무엇인지 제목까지 적는다
  postNothingFound  아무것도 못 찾았을 때. 조용히 넘어가면 같은 것을 몇 번이고 다시 끌어다 놓는다
  미확인 초안       앱을 나간 뒤 1시간
