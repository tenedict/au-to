...
**이 파일 안의 호스트 이름은 제품 이름이 아니라 살아 있는 인프라 주소다.**
이름 바꾸기·일괄 치환이 여기를 건드리면 안 된다 — 커밋되지 않으므로 리뷰가 못 잡고,
사용자에게는 "스크린샷 내용을 분석 요청으로 보낼 수 없어요"(404) 만 보인다.
검사기 규칙 14 가 그 뒤로 이 주소를 DNS 에서 확인한다.

**앱과 서버는 함께 배포되지 않는다.** 둘 사이의 약속(인증 헤더 이름·API 경로)을 바꿀 때는
**옛 것을 함께 받는 기간**을 반드시 둔다. 앱만 새 이름을 보내면 배포된 서버가 401 을 주고,
사용자는 방금 업데이트한 앱에서 "앱을 업데이트해 주세요" 를 본다. 규칙 13 이 짝을 대조한다.
...
물방울 드롭 · Dock · 파인더 ↘
                            CaptureQueue → TaskStore.intake → 알림
Share Extension → App Group inbox ↗   (앱이 다음에 열릴 때 이어서 처리)
사진 고르기(PhotosPicker) ↗
```

**읽기는 화면이 들고 있지 않다.** `CaptureQueue` 가 앱과 함께 살면서 줄을 세우고
속도를 지키고(`IntakeRate` · 1분에 10개) 끝나면 알린다. 화면에 두면 창이 열리고
닫힐 때 처리가 끊겨, 사용자에게는 **등록이 취소된 것**으로 보인다 (실제로 그랬다).
...
1. **무엇이 들어오든 등록한다. 사람이 판단하는 자리는 저장 뒤다.**
   애매해도 등록하고, 봐야 할 이유를 `AssistantTask.reviewReason` 으로 **붙인다** — 막지 않는다.
   막아 두면 사용자는 아무 일도 일어나지 않은 것을 보고, 그게 이 제품이 없애려던 상태다.
   대신 **등록된 것은 언제 어디서든 고치고 지울 수 있어야 하고**, 등록했다고 반드시 알린다.
   Apple 캘린더만 예외다 (`AutoFilePolicy.mayAutoAddToCalendar`) — 밖으로 나가는 출력이라 무게가 다르다
...
11. **알림 예약은 정해진 두 파일에서만.** 마감은 `LocalNotificationService`, 나머지 전부는 `CaptureNotice`.
...
16. **표식은 하나뿐이다** — `Mark`(둥근 사각형 + 빗금 셋). 앱 아이콘·메뉴바·잠금화면
    위젯·홈 위젯·떠 있는 창이 전부 이것을 쓴다. 자리마다 다르게 그리면 같은 앱으로 보이지 않는다
17. **색은 `Palette` 의 둘뿐이다** — `water`(조작 가능·선택·오늘)와 `past`(마감 경과). 나머지는 무채색이다.
    **마감 묶음마다 색을 주지 않는다** — 순서가 이미 급함을 말하므로 색은 규칙을 되풀이할 뿐이다.
    치수는 `CardMetrics`, 간격은 `Space` 에서 가져온다. 화면에 숫자를 직접 적지 않는다
18. **저장소가 서비스를 직접 만들지 않는다.** 캘린더·알림·OCR 은 전부 주입받는다.
    `CalendarService()` 를 직접 만들었더니 단위 테스트가 EventKit 권한 요청에 걸려
    **스위트가 583초** 걸렸다 (지금은 0.2초). 느린 테스트는 아무도 안 돌린다
19. 화면 파일은 500줄 이내
20. `#if DEBUG` 코드는 릴리스 빌드에 새지 않는다
21. **`.sheet` 를 한 뷰에 둘 이상 달지 않는다.** SwiftUI 는 표시 하나만 지원하고,
    겹쳐 달면 **조용히 무시된다** — 알림을 눌러도 편집기가 안 떴다. 라우팅은 열거형 하나로
22. **읽기는 백그라운드 작업으로 감싼다.** `CaptureQueue` 가 `BackgroundActivity` 를
    잡지 않으면 사진을 찍고 앱을 나가는 순간 분석이 끊긴다 — 그게 이 제품의 정상 동선이다
...
**흐름은 셋이다** — 공유(또는 드롭) → 등록 → 알림. 예전의 "앱 열기 → 확인" 두 단계를 없앴다.
확인을 버린 것은 아니다. 날짜가 모호하면 등록하지 않고 확인을 요청한다 (`AutoFilePolicy`).
**어느 쪽이든 창을 스스로 열지 않는다** — 알림으로 부르고, 사용자가 올 때 보여준다.

**고치는 문은 하나다** — `TaskEdit` 이 규칙을 갖고, iOS·macOS 가 그 값을 쓴다.
목록·캘린더·확인 필요·**새로 적기**가 전부 같은 편집기를 지난다.
나눠 만들면 한쪽에만 있는 항목이 반드시 생긴다.

**확인 대기 초안이라는 상태는 없다.** 분석 결과는 곧바로 할 일이 되고,
봐야 할 것은 `reviewReason` 으로 그 할 일에 붙어 다닌다. 사용자가 확인하면 사라진다.
...
등록 알림  CaptureNotice.postFiled · 흐름의 마지막 단계 · 식별자 앞에 "capture#"
  공유·드롭 직후   "언제 무슨 일정이 등록되었습니다"
  문구에 언제와 무엇이 다 들어간다 — 없으면 앱을 열어 확인해야 하고 단계를 줄인 의미가 없다
  봐야 할 것이 있으면 **같은 알림 안에서** 덧붙인다. 나누면 한 번 놓았는데 알림이 둘 온다
  userInfo 에 **taskID 를 반드시 싣는다** — 없으면 눌러도 목록만 뜨고,
  사용자에게는 "눌러도 안 들어가진다" 로 보인다 (실제로 그랬다)

못 찾음    CaptureNotice.postNothingFound
  조용히 넘어가면 같은 스크린샷을 몇 번이고 다시 끌어다 놓는다. 실패도 결과다
```
...
· **iOS 테스트 182건 · 백엔드 테스트 49건 · 빌드 경고 0**

```
공유 · 드롭 · Dock · 카메라 → 줄(CaptureQueue) → OCR → 분석 → **언제나 등록** → 알림
직접 적기 ────────────────────────────────────────────────↗   ↘ 애매하면 "확인 필요" 표식
```

iOS 는 공유·위젯·카메라로 받는다 — [25장](docs/platform/25-IOS.md).
macOS 는 떠 있는 창·Dock 으로 받고 사이드바 대시보드로 본다 — [18장](docs/platform/18-MACOS.md).
