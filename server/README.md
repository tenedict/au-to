...
### 살아 있는 것들의 이름 — 제품 이름과 다릅니다

| | 값 | |
| --- | --- | --- |
| 프로젝트 | `capturetask-260801` | |
| 서비스 | `capturetask-backend` | URL 이 여기서 나옵니다 |
| 지역 | `asia-northeast3` (서울) | |
| 시크릿 | `OPENAI_API_KEY` · `CAPTURETASK_CLIENT_KEY` | 자원 이름 |
| 컨테이너 환경변수 | `OPENAI_API_KEY` · `WHENLY_CLIENT_KEY` | 코드가 읽는 이름 |

**시크릿의 자원 이름과 환경변수 이름이 다릅니다.** Cloud Run 의 `--set-secrets` 가
`환경변수=자원이름:버전` 매핑을 해 주므로, GCP 안의 이름을 건드리지 않고 서버 코드만
새 이름을 읽습니다.

> ⚠️ **이 이름들을 제품 이름에 맞춰 바꾸지 마세요.**
> 서비스 이름을 바꾸면 기존 서비스를 고치는 대신 **새 서비스가 하나 더 생기고**,
> 앱은 그대로 옛 주소를 봅니다 — 배포는 성공하는데 앱에는 아무것도 안 바뀝니다.
> 실제로 CaptureTask → Whenly 일괄 치환이 이 값들을 함께 바꿔서, 존재하지 않는
> 서비스로 요청이 가 404 를 받았습니다.
>
> 옮기고 싶으면 새로 배포한 뒤 `config/apple/Secrets.xcconfig` 의 `WHENLY_HOST` 를
> 새 URL 로 **함께** 바꿉니다. 검사기 규칙 14 가 그 둘이 어긋났는지 봅니다.
...
이름을 다르게 쓰려면 스크립트에 알려 줍니다.

```bash
WHENLY_SERVICE=<서비스명> WHENLY_CLIENT_KEY_SECRET=<시크릿명> ./scripts/deploy-backend.sh
```
...
둘 사이의 약속(헤더 이름 · 경로 · 필드)을 바꿀 때는 **옛 것을 함께 받는 기간**을
반드시 둡니다. 서버가 새 이름만 받으면 아직 옛 빌드가 깔린 폰은 전부 401 을 받고,
사용자에게는 "앱을 업데이트해 주세요" 가 뜨는데 스토어에는 아직 새 빌드가 없습니다.

지금 남아 있는 것 — `x-capturetask-key`. 서버만 받습니다. 앱은 이미 새 이름을
보냅니다. 지우는 조건은 **옛 이름으로 오는 요청이 없는 것을 로그로 확인한 뒤**입니다.
방향이 반대라 지우는 순서도 다릅니다 (자세히는 [21장 §3.1](../docs/engineering/21-API.md)).

### 콜드 스타트를 계산에 넣으세요

`--min-instances 0` 이라 **첫 요청은 컨테이너가 뜨기를 기다립니다** (수 초).
거기에 OpenAI 왕복이 더해집니다. 앱의 요청 제한시간이 25초인 것은 그 합을
넘기지 않으려는 값입니다 — 20초로 뒀을 때 실제로 "요청 시간이 다 되었어요" 가 떴습니다.

돈을 더 내고 `--min-instances 1` 로 두면 사라지는 문제입니다. 지금은 안 그럽니다.
