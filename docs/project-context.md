...
**Whenly** — 스크린샷을 공유하거나 물방울에 떨어뜨리면 일정이 **바로 등록되고**,
등록됐다고 알려 주는 iOS · macOS 개인 비서 앱.

```
1. 담는다      공유 시트 · 물방울 드롭 · Dock · 파일 고르기 · 잠금화면 위젯 → 카메라
               (먼저 App Group 상자에 담는다. 죽어도 잃지 않기 위해서다)
2. 줄에 선다   CaptureQueue 가 앱과 함께 살면서 하나씩 처리한다. 1분에 10건까지
3. 읽는다      Apple Vision 으로 온디바이스 OCR
4. 이해한다    백엔드가 OpenAI 로 제목·마감·시간·신뢰도·근거를 뽑는다
5. 등록한다    분명하면 확인 없이 할 일 + 캘린더 + 마감 알림까지
6. 알린다      "언제 무슨 일정이 등록되었습니다" 푸시 알림
               모호하면 등록하지 않고 "언제인지 확인해 주세요"
```

> **AI 는 제안한다. 사람이 최종 판단을 하는 자리를 저장 앞에서 저장 뒤로 옮겼을 뿐이다.**
> 그래서 **등록된 것은 언제 어디서든 고치고 지울 수 있어야 한다** — 목록에서도,
> 캘린더에서도, 두 플랫폼 어디서도. 고칠 수 없는 자동 등록은 남이 쓴 일정이다.
...
| 캘린더 탭은 **원장이 아니다** | 같은 원장을 날짜로 본 것이다. 거기서도 고칠 수 있지만 **저장 경로는 하나**(`TaskStore`)다 |
| 고치는 문은 **하나** | `TaskEdit` 이 규칙을 갖고 iOS·macOS 가 그 값을 쓴다. 나눠 만들면 한쪽에만 있는 항목이 생긴다 |
| 읽기는 **화면이 들고 있지 않다** | `CaptureQueue` 가 앱과 함께 산다. 화면에 두면 창이 닫힐 때 처리가 끊겨 등록이 취소된 것으로 보인다 |
| 창을 **스스로 열지 않는다** | 사용자는 다른 일을 하는 중이다. 알림으로 부르고, 올 때 보여준다 |
...
| 항목 | 값 |
| --- | --- |
| 플랫폼 | iOS 17.0+ / iPhone · macOS 14.0+ |
| 언어·UI | Swift 5, SwiftUI |
| 상태 관리 | `@MainActor` + `ObservableObject` |
| 프레임워크 | SwiftUI · Vision · EventKit · UserNotifications · WidgetKit · UIKit/AppKit(확장) |
| 외부 의존성 | **없음** (iOS·백엔드 모두) |
| 저장 | App Group / JSON (원자적 쓰기) + 파일 상자. 옛 위치·옛 이름은 자동으로 가져온다 |
| 백엔드 | Node 22 · `node:http` · 외부 패키지 0 |
| 타깃 | Whenly(iOS) · WhenlyShare · WhenlyWidgets · WhenlyMac · WhenlyMacShare · WhenlyTests |
| 프로젝트 정의 | XcodeGen (`project.yml`) — `.xcodeproj` 는 **생성물, 미추적** |
| 테스트 | XCTest 176건 · `node:test` 46건 |
| 번들 ID | `com.example.whenly` (D-1 미확정) |
| App Group | `group.com.example.whenly` |
| URL 스킴 | `whenly://capture` · `whenly://tasks` (`DeepLink`) |
| 기본 언어 | 한국어 |
