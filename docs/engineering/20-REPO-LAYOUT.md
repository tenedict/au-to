...
**`Design/` 은 화면이 아니라 공용이다.** 색·간격·치수는 그리는 코드가 아니라 **값**이고,
같은 값을 두 플랫폼이 써야 한다. `apps/ios/Views` 에 두면 macOS 가 같은 숫자를 다시 적게 되고,
그 순간 두 앱의 카드 높이가 조용히 갈라진다. 실제로 마감 텍스트가 그렇게 갈라져 카드가 잘렸다.
